...
void add_to_log(ScrollPanel *panel, const char *text) {
    ...
    if (panel->line_count < 1000) panel->line_count++;
}

ScrollPanel *create_log_panel(int height, int width, int y, int x, const char *title) {
    ...
    (void)y; (void)x; (void)title; // Silence unused parameter warnings
    return &panel;
}
